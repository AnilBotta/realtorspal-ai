...
# ===========================
# Draft Activity Helper Functions (Wrappers)
# ===========================

# These functions now call the implementations from draft_activity_manager module
async def update_draft_activity(lead_id: str, user_id: str, channel: str):
    """Wrapper for update_draft_activity from draft_activity_manager"""
    return await update_draft_activity_impl(lead_id, user_id, channel)

# Add endpoint to manually trigger activity sync (useful for testing)
@app.post("/api/draft-activities/sync/{lead_id}")
async def sync_draft_activities(lead_id: str, user_id: str):
    """Manually sync draft activities for a lead"""
    try:
        lead = await db.leads.find_one({"id": lead_id, "user_id": user_id})
        if not lead:
            raise HTTPException(status_code=404, detail="Lead not found")
        
        # Sync both email and SMS activities
        await update_draft_activity(lead_id, user_id, "email")
        await update_draft_activity(lead_id, user_id, "sms")
        
        return {"status": "success", "message": "Draft activities synced"}
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
