...
export function MetricsOverview() {
  ...
  const { backendConnected, user } = useAuth()
  ...
  // Demo engagement metrics for admin, real/empty metrics for others
  const engagementMetrics = isAdmin ? [
    ...
    { label: "Email Open Rate", value: metrics?.totalLeads > 0 ? Math.round((metrics?.totalLeads || 0) * 0.3) : 0, target: 70 },
    { label: "SMS Click Rate", value: metrics?.activeConversations > 0 ? Math.round((metrics?.activeConversations || 0) * 0.5) : 0, target: 40 },
    { label: "Call Connection Rate", value: metrics?.appointmentsScheduled > 0 ? Math.round((metrics?.appointmentsScheduled || 0) * 2.4) : 0, target: 80 },
    { label: "Document Completion", value: metrics?.conversionRate > 0 ? Math.round(metrics?.conversionRate * 4) : 0, target: 85 }
  ]
